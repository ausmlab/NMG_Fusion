    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        for key in batch[0] :
            if key == 'target' :
                batch_list = []
                for data in batch:
                    batch_list.append(data['target'])
                inputs['target'] = batch_list
            elif key == 'offset' :
                inputs['offset'] = torch.cumsum(collate_fn([d[key] for d in batch]), dim=0)
            else :
                inputs[key] = collate_fn([d[key] for d in batch])
        return inputs
    else:
