...
from pointcept.utils.eval2d import evaluate_oriented_detection
...
class SemSegEvaluator(HookBase):
    def after_epoch(self):
        if self.trainer.cfg.evaluate:
            self.num_select = self.trainer.cfg.num_select
            self.iou_threshold = self.trainer.cfg.iou_threshold
            self.score_threshold = self.trainer.cfg.score_threshold
            self.eval()
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            if 'target' in input_dict.keys():
                for i in range(len(input_dict['target'])):
                    for key in input_dict['target'][i].keys():
                        if isinstance(input_dict['target'][i][key], torch.Tensor):
                            input_dict['target'][i][key] = input_dict['target'][i][key].cuda(non_blocking=True)
            with torch.no_grad():
                output_dict = self.trainer.model(input_dict)
            ...
            # 2D
            if not self.trainer.model.weight_2d == 0:
                loss_2d = output_dict["loss2d"]

                output2d = output_dict['det_out']
                targets2d = [{k: v for k, v in t.items()} for t in input_dict['target']]

                target_sizes = torch.stack([t["orig_size"] for t in targets2d], dim=0)

                num_select = self.num_select  # 300  ###<--------------------
                out_logits, out_bbox = output2d['pred_logits'], output2d['pred_boxes']
                ...
                for i, (tgt, res) in enumerate(zip(targets2d, results)):
                    """
                    pred vars:
                    ...
                    angle_denorm_fct = torch.tensor([[0., 0., 0., 0., 0.5]]).to(img_h.device)
                    gt_bbox = gt_bbox - angle_denorm_fct

                    theta = torch.tensor([torch.pi]).to(img_h.device)
                    scale_fct = torch.stack([img_w[None], img_h[None], img_w[None], img_h[None], theta], dim=1)
                    gt_bbox = gt_bbox * scale_fct
                    gt_info = torch.cat((gt_bbox, gt_label.unsqueeze(-1)), 1)  # cx,cy,w,h,theta,label

                    _res_bbox = res['boxes']  # 300, 5
                    _res_prob = res['scores']  # 300
                    _res_label = res['labels']  # 300
                    res_info = torch.cat((_res_bbox, _res_prob.unsqueeze(-1), _res_label.unsqueeze(-1)),
                                         1)  # cx,cy,w,h,theta,score,label
                ...
                self.trainer.storage.put_scalar("val_loss_2d", loss_2d.item())
                info = info + " Loss2D {loss2d:.4f}".format(loss2d=loss_2d.item())
                #self.trainer.logger.info(
                #    info
                #    + "Loss2D {loss2d:.4f}".format(
                #        loss2d=loss_2d.item()
                #    )
                #)

            # 3D
            if not self.trainer.model.weight_3d == 0 :
                output = output_dict["seg_logits"]
                loss_3d = output_dict["loss3d"]
                pred = output.max(1)[1]
                ...
                self.trainer.storage.put_scalar("val_union", union)
                self.trainer.storage.put_scalar("val_target", target)
                self.trainer.storage.put_scalar("val_loss_3d", loss_3d.item())

                info = info + " Loss3D {loss3d:.4f}".format(loss3d=loss_3d.item())
                #self.trainer.logger.info(
                #    info
                #    + "Loss3D {loss3d:.4f}".format(
                #        loss3d=loss_3d.item()
                #    )
                #)
        ...
        # 3D
        if not self.trainer.model.weight_3d == 0 :
            loss_3d_avg = self.trainer.storage.history("val_loss_3d").avg
            intersection = self.trainer.storage.history("val_intersection").total
            union = self.trainer.storage.history("val_union").total
            target = self.trainer.storage.history("val_target").total
            iou_class = intersection / (union + 1e-10)
            ...
            self.trainer.logger.info(
                "3D Val result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.".format(
                    m_iou, m_acc, all_acc
                )
            )
            for i in range(self.trainer.cfg.data.num_classes):
        ...
        # 2D
        if not self.trainer.model.weight_2d == 0 :
            loss_2d_avg = self.trainer.storage.history("val_loss_2d").avg
            result = evaluate_oriented_detection(output_state_dict['gt_info'], output_state_dict['res_info'],
                                                 iou_threshold=self.iou_threshold,
                                                 score_threshold=self.score_threshold)

            ap_pylon = result['pylon']  ###<----- we don't need stats
            ap_span = result['powerline']  ###<-----
            map = result['mAP']  ###<-----


            self.trainer.logger.info(
                "2D Val result: pylon/span/map {:.4f}/{:.4f}/{:.4f}.".format(
                    ap_pylon, ap_span, map
                )
            )
        ...
        if self.trainer.writer is not None:
            self.trainer.writer.add_scalar("val/loss", loss_avg, current_epoch)
            if not self.trainer.model.weight_3d == 0:
                self.trainer.writer.add_scalar("val/loss3d", loss_3d_avg, current_epoch)
                self.trainer.writer.add_scalar("val/mIoU", m_iou, current_epoch)
                self.trainer.writer.add_scalar("val/mAcc", m_acc, current_epoch)
                self.trainer.writer.add_scalar("val/allAcc", all_acc, current_epoch)
            if not self.trainer.model.weight_2d == 0:
                self.trainer.writer.add_scalar("val/loss2d", loss_2d_avg, current_epoch)
                self.trainer.writer.add_scalar("val/ap_pylon", ap_pylon, current_epoch)
                self.trainer.writer.add_scalar("val/ap_span", ap_span, current_epoch)
                self.trainer.writer.add_scalar("val/map", map, current_epoch)
        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
        ...
        if not self.trainer.model.weight_3d == 0:
            ...
            self.trainer.comm_info["current_metric_value"] = map  # save for saver
            self.trainer.comm_info["current_metric_name"] = "mAP"  # save for saver
    def after_train(self):
        self.trainer.logger.info(
            "Best {}: {:.4f}".format("{}".format(self.trainer.comm_info["current_metric_name"]), self.trainer.best_metric_value)
        )
