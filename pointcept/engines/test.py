    ...
    def test(self):
                ...
                for i in range(len(fragment_list)):
                    ...
                    with torch.no_grad():
                        ...
                        if self.cfg.weight_2d != 0:
                            ...
                            num_select = self.cfg.num_select  # 300  ###<--------------------
                            ...
                            boxes = torch.gather(boxes, 1,
                                                 topk_boxes.unsqueeze(-1).repeat(1, 1, 5))  ###<----------
                            ...
                            for i, (tgt, res) in enumerate(zip(targets2d, results)):
                                ...
                                scale_fct = torch.stack(
                                    [img_w[None], img_h[None], img_w[None], img_h[None], theta], dim=1)
                                gt_bbox = gt_bbox * scale_fct
                                gt_info = torch.cat((gt_bbox, gt_label.unsqueeze(-1)),
                                                    1)  # cx,cy,w,h,theta,label

                                _res_bbox = res['boxes']  # 300, 5
                                _res_prob = res['scores']  # 300
                                _res_label = res['labels']  # 300
                                res_info = torch.cat(
                                    (_res_bbox, _res_prob.unsqueeze(-1), _res_label.unsqueeze(-1)),
        ...
        if comm.is_main_process():
            ...
            if self.cfg.weight_3d != 0 :
                ...
                logger.info(
                    "3D Test result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}".format(
                        mIoU, mAcc, allAcc
                    )
                )
            ...
            if self.cfg.weight_2d != 0:
                OBB_save_path = os.path.join(save_path, 'OBB')
                make_dirs(OBB_save_path)
                # OBB evaluation
                with open(os.path.join(OBB_save_path, 'gt.pkl'), 'wb') as f:
                    pickle.dump(output_state_dict['gt_info'], f)
                with open(os.path.join(OBB_save_path, 'pred.pkl'), 'wb') as f:
                    pickle.dump(output_state_dict['res_info'], f)
                ...
                map = result['mAP']  ###<-----
                logger.info(
                    "2D Test result: Pylon/Span/mAP {:.4f}/{:.4f}/{:.4f}".format(
