...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        two_stage_class_embed_share=True,  # False
        two_stage_bbox_embed_share=True,  # False
        d_model=128,
        dropout=0.0,
        nhead=8,
        num_queries=600,
        ...
        self.hidden_dim = d_model ##,- should be an arg
        self.return_interm_indices = return_interm_indices
        self.in_channels = enc_channels
        num_feature_levels =  len(self.return_interm_indices)
        ...
        self.num_classes_2d = num_classes_2d
        self.label_enc = nn.Embedding(dn_labelbook_size + 1, self.hidden_dim)

        input_proj_list = []
        for i in self.return_interm_indices:  ## should be changed with interm
            input_proj_list.append(nn.Sequential(
                nn.Conv2d(self.in_channels[i], self.hidden_dim, kernel_size=1),
        ...
        self.position_embedding = PositionEmbeddingSineHW(
            N_steps,
            temperatureH=self.pe_temperatureH,
            temperatureW=self.pe_temperatureW,
            normalize=True
        )
        ...
        self.transformer = DeformableTransformer(
                                ...
                                num_unicoder_layers=num_unicoder_layers,
        ...
        if not weight_2d == 0 :
            ...
            _class_embed = nn.Linear(self.hidden_dim, num_classes_2d)
            _bbox_embed = MLP2D(self.hidden_dim, self.hidden_dim, 5, 3)  ##<----- MLP(hidden_dim, hidden_dim, 4, 3)
            # init the two embed layers
            prior_prob = 0.01
            bias_value = -math.log((1 - prior_prob) / prior_prob)
            _class_embed.bias.data = torch.ones(num_classes_2d) * bias_value
            nn.init.constant_(_bbox_embed.layers[-1].weight.data, 0)
            nn.init.constant_(_bbox_embed.layers[-1].bias.data, 0)

            if dec_pred_bbox_embed_share:  # True
                box_embed_layerlist = [_bbox_embed for i in range(self.transformer.num_decoder_layers)]  # [_bbox_embed] * 6
            else:
            ...
            if dec_pred_class_embed_share:  # True
                class_embed_layerlist = [_class_embed for i in range(self.transformer.num_decoder_layers)]  # [_class_embed] * 6
            else:
            ...
            if two_stage_type != 'no':
                ...
                if self.two_stage_add_query_num > 0:  # 0
                    self.init_ref_points(two_stage_add_query_num)

            self.decoder_sa_type = decoder_sa_type  # sa
            assert decoder_sa_type in ['sa', 'ca_label', 'ca_content']
            if decoder_sa_type == 'ca_label':
                self.label_embedding = nn.Embedding(num_classes_2d, self.hidden_dim)
    ...
    def forward(self, data_dict, w_3d, w_2d):
        ...
        for s in range(self.num_stages):
            ...
            if s in self.return_interm_indices : ##<----------
                feat, mask = self.extract_2d_feature_grids(point, s, self.tile_size, self.grid_size)
                token = NestedTensor(feat, mask)
                tokens.append(token) ###<---------------
        ...
        if not w_2d == 0 :
            ...
            masks = []
            poss = []
            for l, feat in enumerate(tokens):
            ...
            if self.dn_number > 0 or targets is not None: # self.dn_number: 100
                input_query_label, input_query_bbox, attn_mask, dn_meta =\
                    prepare_for_cdn(dn_args=(targets, self.dn_number, self.dn_label_noise_ratio, self.dn_box_noise_scale),
                                    training=self.training,num_queries=self.num_queries,num_classes=self.num_classes_2d,
    ...
    def extract_2d_feature_grids(self, point, s, tile_size, grid_size):
        """
        Build 2D feature grids from point clouds with optional augmentation such as flipping and scaling.

        Args:
            point: An object with attributes
            s (int): scaling exponent, base grid size * (2**s).
            tile_size: tile size of point cloud on x-y plane
            grid_size: size used for grid_sampling
        Returns:
            list[Tensor]: list of feature grids, each with shape (C, out_size, out_size).
        """

        start_point = 0

        # Define grid_size (controls resolution of spatial bins)
        grid_size = grid_size * (2 ** s)  ##<- should be an arg
        grid_size = torch.tensor(grid_size, device=point.coord.device, dtype=point.coord.dtype)

        feats = []
        masks = []
        # Loop over each "batch" defined by offset
        for i, offset in enumerate(point.offset):
            coord = point.coord[start_point:offset]  # slice coords for this batch
            H, W = point.target[i]['size'] # rescaled size

            # Handle horizontal and vertical flips
            ...
            # Grid coordinates with minx and miny
            grid_coord = point.grid_coord[start_point:offset]
            grid_x = grid_coord[:, 0] + minx
            grid_y = grid_coord[:, 1] + miny

            # Normalize sizes to keep consistent scale
            size = math.ceil(tile_size / grid_size)  # should be a arg.
            ...
            # Clamp coordinates so they don't exceed grid
            grid_x = torch.clamp(grid_x, max=size - 0.5)  # should be self.grid_size /2
            grid_y = torch.clamp(grid_y, max=size - 0.5)  # should be self.grid_size /2

            # Features for this slice
            grid_feat = point.feat[start_point:offset]
            N, C = grid_feat.shape
            ...
            grid_sum = torch.zeros((C, size, size), dtype=grid_feat.dtype, device=grid_feat.device)
            grid_count = torch.zeros((1, size, size), dtype=grid_feat.dtype, device=grid_feat.device)

            # Accumulate feature values into grid
            for c in range(C):
            ...
            mask = torch.zeros_like(grid_count, dtype=torch.bool) # shape (1, size, size)

            # Average pooling per cell
            grid_feat = grid_sum / (grid_count + 1e-6)
            feats.append(grid_feat)
            masks.append(mask)

            start_point = offset

        return torch.stack(feats, dim=0), torch.cat(masks, dim=0)
