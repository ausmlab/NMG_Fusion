...
def gen_encoder_output_proposals(memory:Tensor, memory_padding_mask:Tensor, spatial_shapes:Tensor, learnedwh=None):
    ...
    Output:
        ...
        - output_proposals: bs, \sum{hw}, 5  ##<-------bs, \sum{hw}, 4
    """
    N_, S_, C_ = memory.shape
    base_scale = 4.0
    proposals = []
    _cur = 0
    for lvl, (H_, W_) in enumerate(spatial_shapes): # tensor[[128, 128], [ 64,  64], [ 32,  32], [ 16,  16]]
        ...
        if learnedwh is not None: # None
            wh = torch.ones_like(grid) * learnedwh.sigmoid() * (2.0 ** lvl)
        else:
            wh = torch.ones_like(grid) * 0.05 * (2.0 ** lvl)
        theta = torch.ones_like(grid_x).expand(N_, -1, -1).unsqueeze(-1) * 0.5 ###<--# set default angle to 0 radian
        proposal = torch.cat((grid, wh, theta), -1).view(N_, -1, 5)  ###<-----------torch.cat((grid, wh), -1).view(N_, -1, 4)
...
def gen_sineembed_for_position(pos_tensor, size):
    # n_query, bs, _ = pos_tensor.size()
    # sineembed_tensor = torch.zeros(n_query, bs, 256)
    scale = 2 * math.pi
    dim_t = torch.arange(size, dtype=torch.float32, device=pos_tensor.device)
    dim_t = 10000 ** (2 * (dim_t // 2) / size)
    ...
    if pos_tensor.size(-1) == 2:
        ...
        ### source: https://github.com/SIAnalytics/RHINO/blob/main/mmrotate/models/layers/transformer/utils.py#L11
        theta = pos_tensor[..., 4]
