...
from .utils import inverse_sigmoid, gen_encoder_output_proposals, MLP,_get_activation_fn, gen_sineembed_for_position
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.num_unicoder_layers = num_unicoder_layers # 0
        self.num_decoder_layers = num_decoder_layers # 6
        self.deformable_encoder = deformable_encoder # True
        self.deformable_decoder = deformable_decoder # True
        ...
        self.use_detached_boxes_dec_out = use_detached_boxes_dec_out # False
        assert query_dim == 5 ##<---- 4

        if num_feature_levels > 1:
            assert deformable_encoder, "only support deformable_encoder for num_feature_levels > 1"
        if use_deformable_box_attn: # False
            assert deformable_encoder or deformable_encoder

        assert layer_share_type in [None, 'encoder', 'decoder', 'both'] # None
        if layer_share_type in ['encoder', 'both']:
            enc_layer_share = True
        else:
            enc_layer_share = False
        if layer_share_type in ['decoder', 'both']: # None
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        Input:
            ...
            - refpoint_embed: [bs, num_dn, 5]. None in infer  ###<--- [bs, num_dn, 4]
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            pos_embed = pos_embed.flatten(2).transpose(1, 2)    # bs, hw, c(256)
            if self.num_feature_levels > 1 and self.level_embed is not None:
        ...
        if self.two_stage_type =='standard':
            ...
            if self.two_stage_pat_embed > 0: # 0 (skip)
            ...
            if self.two_stage_add_query_num > 0: # 0 (skip)
            ...
            enc_outputs_coord_unselected = self.enc_out_bbox_embed(output_memory) + output_proposals # (bs, \sum{hw}, 5) unsigmoid
            topk = self.num_queries
            topk_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], topk, dim=1)[1] # bs, nq

            # gather boxes
            refpoint_embed_undetach = torch.gather(enc_outputs_coord_unselected, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 5)) # unsigmoid  ##<---------
            refpoint_embed_ = refpoint_embed_undetach.detach() # bs, k, 5
            init_box_proposal = torch.gather(output_proposals, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 5)).sigmoid() # bs, k, 5 sigmoid ##<-------

            # gather tgt
            tgt_undetach = torch.gather(output_memory, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model)) # bs, k, 256
            if self.embed_init_tgt: # True
        ...
        hs, references = self.decoder(
                tgt=tgt.transpose(0, 1), # 900+200, bs, 256
                memory=memory.transpose(0, 1), # sum(wi*hi), bs, 256
                memory_key_padding_mask=mask_flatten, 
                pos=lvl_pos_embed_flatten.transpose(0, 1), #  sum(wi*hi), bs, 25
                refpoints_unsigmoid=refpoint_embed.transpose(0, 1), # 900+200, bs, 5
                level_start_index=level_start_index, 
                spatial_shapes=spatial_shapes,
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, 
        ...
        intermediate_output = []
        intermediate_ref = []
        if ref_token_index is not None: # skip
        ...
        for layer_id, layer in enumerate(self.layers):
            # main process
            dropflag = False
            if self.enc_layer_dropout_prob is not None: # skip
                prob = random.random()
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, 
        ...
        assert query_dim in [2, 4, 5], "query_dim should be 2/4 but {}".format(query_dim)
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.training and self.decoder_query_perturber is not None and layer_id != 0: # skip
                reference_points = self.decoder_query_perturber(reference_points)

            if self.deformable_decoder: # True
                if reference_points.shape[-1] == 5: ##<----------  4
                    dummy_angle = torch.ones_like(valid_ratios[..., :1]) * torch.pi
                    reference_points_input = reference_points[:, :, None] \
                                             * torch.cat([valid_ratios, valid_ratios, dummy_angle], -1)[None, :]  # nq, bs, nlevel, 5
                    #reference_points_input = reference_points[:, :, None] \
                    #                        * torch.cat([valid_ratios, valid_ratios, valid_ratios[:,:,:1]], -1)[None, :] # nq, bs, nlevel, 5 / 4
                else:
                    assert reference_points.shape[-1] == 2
                    reference_points_input = reference_points[:, :, None] * valid_ratios[None, :]
                # skip angle dim (https://github.com/SIAnalytics/RHINO/blob/d9baf3bc1149ebef2ef03c89b26b93106b82075a/mmrotate/models/layers/transformer/rhino_layers_v2.py#L105)
                query_sine_embed = gen_sineembed_for_position(reference_points_input[:, :, 0, :4], tgt.size(-1)//2) # nq, bs, 256*2
            else:
            ...
            if not self.deformable_decoder: # Skip
                query_sine_embed = query_sine_embed[..., :self.d_model] * self.query_pos_sine_scale(output)

            # modulated HW attentions
            if not self.deformable_decoder and self.modulate_hw_attn: # Skip
            ...
            dropflag = False
            if self.dec_layer_dropout_prob is not None: # Skip
                prob = random.random()
                if prob < self.dec_layer_dropout_prob[layer_id]:
                    dropflag = True
            if not dropflag: # False
                output = layer(
                    tgt = output,
            ...
            if self.bbox_embed is not None:
                ...
                # select # ref points
                if self.dec_layer_number is not None and layer_id != self.num_layers - 1: # skip
                ...
                if self.rm_detach and 'dec' in self.rm_detach:
                    ...
                    reference_points = new_reference_points.detach() ## This one is used
                if self.use_detached_boxes_dec_out:
                    ref_points.append(reference_points)
                else:
                    ref_points.append(new_reference_points) # This one is used
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        for funcname in self.module_seq: # ['sa', 'ca', 'ffn']
            if funcname == 'ffn':
...
def build_deformable_transformer(args):
    decoder_query_perturber = None
    if args.decoder_layer_noise: # Skip
    ...
    use_detached_boxes_dec_out = False
    try:
        use_detached_boxes_dec_out = args.use_detached_boxes_dec_out # False
    except:
        use_detached_boxes_dec_out =False
